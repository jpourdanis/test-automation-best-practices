  ...
  test('Frontend package.json should not have HIGH/CRITICAL vulnerabilities', async ({}, testInfo) => {
    ...
    try {
      ...
      if (e.stdout) {
        ...
        expect(
          targetIssues,
          `Found ${metadata.high} HIGH and ${metadata.critical} CRITICAL vulnerabilities in the frontend`
        ).toBe(0);
      } else {
        throw e;
      }
  ...
  test('Backend server/package.json should not have HIGH/CRITICAL vulnerabilities', async ({}, testInfo) => {
    ...
    try {
      ...
      if (e.stdout) {
        ...
        expect(
          targetIssues,
          `Found ${metadata.high} HIGH and ${metadata.critical} CRITICAL vulnerabilities in the backend API`
        ).toBe(0);
      } else {
        throw e;
      }
