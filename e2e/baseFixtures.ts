...
export const test = baseTest.extend<{ homePage: HomePage; allureBddMapper: void }>({
  ...
  page: async ({ page }, use, testInfo) => {
    ...
    page.on('requestfailed', (request) => {
      console.error(
        `[NETWORK ERROR] ${testInfo.title}: ${request.method()} ${request.url()} - ${request.failure()?.errorText}`
      );
    });
    ...
    page.on('response', (response) => {
      ...
      if (resourceType === 'fetch' || resourceType === 'xhr') {
        if (!response.ok()) {
          console.error(
            `[API ERROR] ${testInfo.title}: ${response.status()} ${response.request().method()} ${response.url()}`
          );
        } else {
          // Captures network when clicking a button that triggers a fetch
          console.log(
            `[NETWORK INFO] ${testInfo.title}: ${response.status()} ${response.request().method()} ${response.url()}`
          );
        }
      }
    });
  ...
  allureBddMapper: [
    ...
    { auto: true },
  ], // The 'auto: true' flag ensures you don't have to call it manually
