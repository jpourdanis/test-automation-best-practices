...
import { readableOn, hexToRgb, hslToRgb, rgbToHex, rgbToHsl, paintWheelPixel, ColorPicker } from './ColorPicker'
  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, string>) => {
      ...
      if (typeof value === 'string' && options) {
        return value.replaceAll(/\{\{(\w+)\}\}/g, (_: string, k: string) => options[k] ?? `{{${k}}}`)
      }
...
describe('Utility Functions', () => {
  ...
  describe('paintWheelPixel', () => {
    function makeImg(pixels = 1): ImageData {
      return { data: new Uint8ClampedArray(pixels * 4) } as unknown as ImageData
    }

    test('sets alpha to 0 when dist > r (outside circle)', () => {
      const img = makeImg()
      paintWheelPixel(img, 0, 15, 0, 15, 10)
      expect(img.data[3]).toBe(0)
    })

    test('paints grey at center (dx=0, dy=0, dist=0, sat=0)', () => {
      const img = makeImg()
      paintWheelPixel(img, 0, 0, 0, 0, 10)
      expect(img.data[0]).toBe(128) // R
      expect(img.data[1]).toBe(128) // G
      expect(img.data[2]).toBe(128) // B
      expect(img.data[3]).toBe(255) // alpha full (dist=0 ≤ r-1)
    })

    test('wraps negative angle: dy<0 gives angle+360 (hue=270 → violet)', () => {
      const r = 10
      const img = makeImg()
      paintWheelPixel(img, 0, 0, -r, r, r)
      // angle = atan2(-r, 0)*180/π = -90 → +360 = 270, s=1
      // hslToRgb(270,1,0.5) = [128,0,255]
      expect(img.data[0]).toBe(128)
      expect(img.data[1]).toBe(0)
      expect(img.data[2]).toBe(255)
    })

    test('anti-aliases the outermost ring: dist=r gives alpha=0', () => {
      const img = makeImg()
      paintWheelPixel(img, 0, 10, 0, 10, 10)
      // a = r - dist = 0 → alpha = 0
      expect(img.data[3]).toBe(0)
    })

    test('anti-aliases near-edge pixel: dist=r-0.5 gives alpha=127', () => {
      const img = makeImg()
      paintWheelPixel(img, 0, 9.5, 0, 9.5, 10)
      // a = r - dist = 0.5 → 0.5*255 = 127.5 → Uint8ClampedArray rounds to 128
      expect(img.data[3]).toBe(128)
    })

    test('paints red at right edge (dx=r, dy=0, hue=0)', () => {
      const r = 10
      const img = makeImg()
      // dist = r-1 so it stays fully opaque but inside
      paintWheelPixel(img, 0, r - 1, 0, r - 1, r)
      // angle=0, s=0.9, hslToRgb(0,0.9,0.5) ≈ red channel dominant
      expect(img.data[0]).toBeGreaterThan(img.data[2]) // R > B
      expect(img.data[3]).toBe(255)
    })

    test('writes to the correct index offset in the array', () => {
      const img = makeImg(3)
      paintWheelPixel(img, 8, 15, 0, 15, 10) // i=8, dist>r → only [8+3]=0
      expect(img.data[11]).toBe(0)
      expect(img.data[3]).toBe(0) // pixel at i=0 untouched α
    })
  })
}) // Utility Functions
...
describe('Component Tests', () => {
  describe('ColorPicker', () => {
    ...
    test('mousemove while dragging updates hex (dragging.current=true kills mutation)', async () => {
      ...
      fireEvent.mouseMove(globalThis as unknown as Window, { clientX: 220, clientY: 110 }) // right → sat=1 → red
    ...
    test('mousemove after mouseup does NOT update color (dragging stopped)', async () => {
      ...
      fireEvent.mouseDown(canvas, { clientX: 220, clientY: 110 })
      fireEvent.mouseUp(globalThis as unknown as Window) // stop dragging
      // Move to top (would give hue=270) — but should NOT update
      fireEvent.mouseMove(globalThis as unknown as Window, { clientX: 110, clientY: 0 })
    ...
    test('mousemove without prior mousedown (not dragging) does not change color', async () => {
      ...
      fireEvent.mouseMove(globalThis as unknown as Window, { clientX: 220, clientY: 110 })
    ...
    test('touchmove while dragging updates hex', async () => {
      ...
      fireEvent.touchMove(globalThis as unknown as Window, { touches: [{ clientX: 220, clientY: 110 }] }) // right → red
    ...
    test('touchend stops dragging (touchmove after touchend is ignored)', async () => {
      ...
      fireEvent.touchEnd(globalThis as unknown as Window)
      // Move to top after end — should NOT update
      fireEvent.touchMove(globalThis as unknown as Window, { touches: [{ clientX: 110, clientY: 0 }] })
